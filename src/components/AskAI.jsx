  ...
  const formatAIResponse = (text) => {
    return text.split("\n").map((line, index) => {
      ...
      if (
        ...
        return (
          <p key={index} className="font-semibold text-white mt-2">
            {trimmed}
          </p>
        );
      }
          ...
          {chat.length === 0 && (
            <div className="p-4 border-b border-gray-200 dark:border-zinc-700">
              ...
              <div className="grid grid-cols-2 gap-2">
                <button onClick={() => sendMessage("Who is Van?")}
                  className="p-3 text-left text-xs rounded-xl bg-zinc-800 text-white hover:bg-zinc-700 transition">
                  👨‍💻 Who is Van?
                </button>

                <button onClick={() => sendMessage("What are his skills?")}
                  className="p-3 text-left text-xs rounded-xl bg-zinc-800 text-white hover:bg-zinc-700 transition">
                  ⚡ Skills & Tech
                </button>

                <button onClick={() => sendMessage("Show me his projects")}
                  className="p-3 text-left text-xs rounded-xl bg-zinc-800 text-white hover:bg-zinc-700 transition">
                  🚀 Projects
                </button>

                <button onClick={() => sendMessage("What experience does he have?")}
                  className="p-3 text-left text-xs rounded-xl bg-zinc-800 text-white hover:bg-zinc-700 transition">
                  💼 Experience
                </button>

                <button onClick={() => sendMessage("What does he do outside development?")}
                  className="p-3 text-left text-xs rounded-xl bg-zinc-800 text-white hover:bg-zinc-700 transition">
                  🏀 Life Outside Dev
                </button>

                <button onClick={() => sendMessage("How can I contact him?")}
                  className="p-3 text-left text-xs rounded-xl bg-zinc-800 text-white hover:bg-zinc-700 transition">
                  📩 Contact
                </button>
              </div>
          ...
          <div className="flex-1 p-4 overflow-y-auto space-y-4">
            {chat.map((msg, i) => (
              ...
              >
                ...
                <div
                  className={`px-4 py-3 rounded-2xl max-w-[75%] text-sm shadow space-y-1 ${
                    msg.sender === "user"
                      ? "bg-black text-white"
